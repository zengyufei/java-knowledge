import com.zyf.cloud.S55_V1_服务注册中心_启动器;
...
@SpringBootTest(classes = S55_V1_服务注册中心_启动器.class)
@AutoConfigureMockMvc
public class S55_V1_服务注册中心_启动器Tests {
    ...
    public void 测试请求2() throws Exception {
        String uri = "/config-client/default/master";
        String content = requestGetAndReturn(uri);
        JSONObject obj = (JSONObject) JSONParser.parseJSON(content);
    ...
    public void 测试请求3() throws Exception {
        String uri = "/config-client/master";
        String content = requestGetAndReturn(uri);
        JSONObject obj = (JSONObject) JSONParser.parseJSON(content);
        Assert.isTrue(obj.getJSONArray("propertySources").length() == 1, "返回值错误");
    }
    ...
    @Test
    public void 测试请求4() throws Exception {
        String uri = "/config-client-dev.yml";
        String content = requestGetAndReturn(uri);
        Assert.isTrue(content.contains("dev-jdbc"), "返回值错误");
    }

    @Test
    public void 测试请求5() throws Exception {
        String uri = "/config-client-default.yml";
        String content = requestGetAndReturn(uri);
        Assert.isTrue(content.contains("default-jdbc"), "返回值错误");
    }
