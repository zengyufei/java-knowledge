...
public class U1_ByteBuf实验 {
    ...
    public static void main(String[] args) {
        // 0::read 读指针位置
        // 9::capacity 实际占用字节数 当前可写的字节数
        // 100::max-capacity 最大长度 最大可写字节数
        ...
        // 往 byteBuf 填充 1 字节，此时写指针在 9，已经与 capacity 相等。
        // 不可写。
        buffer.writeBytes(new byte[]{5});
        print("writeBytes(new byte[]{5})", buffer);

        // 往 byteBuf 填充 1 字节，检查 capacity == max-capacity，如果不相等，则扩容。
        // 扩容后再写。capacity 也改变。
