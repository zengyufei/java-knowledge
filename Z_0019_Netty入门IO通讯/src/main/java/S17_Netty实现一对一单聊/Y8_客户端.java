...
import S17_Netty实现一对一单聊.Y6_处理链.Y14_编码处理器;
import S17_Netty实现一对一单聊.Y6_处理链.Y15_解码处理器;
import S17_Netty实现一对一单聊.Y6_处理链.Y17_登录响应处理器;
import S17_Netty实现一对一单聊.Y6_处理链.Y19_发送消息响应处理器;
import Z_utils.输出;
...
public class Y8_客户端 {
    ...
    public static void 运行(String 用户名) {
        ...
        while (Y13_会话工具类.是否已经登录(通道)) {
            ...
            输出.客户端.控制台输出不换行("请输入需要发送的用户id: ");
            String 对方用户id = 控制台输入监听.next();
            输出.客户端.控制台输出不换行("请输入需要发送的消息内容: ");
            String 消息内容 = 控制台输入监听.next();
            输出.客户端.控制台输出("发送消息[{}]给{}", 消息内容, 对方用户id);
