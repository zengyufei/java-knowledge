...
import S8_Netty实现登录.P0_简易通讯协议.P4_应用层.P8_编码;
import S8_Netty实现登录.P0_简易通讯协议.P4_应用层.P9_解码;
import Z_utils.输出;
...
public class P2_客户端逻辑处理器 extends ChannelInboundHandlerAdapter {
    ...
    private void 打印日志(boolean result) {
        if (!result) {
            输出.客户端.控制台输出(new Date() + ": 解析后说我因为密码错误重新登录");
        } else {
            输出.客户端.控制台输出(new Date() + ": 解析后, 我终于登录成功啦!");
        }
    }
    ...
    private boolean 核对登录是否成功(P7_登录响应数据包 解码后的数据) {
        ...
        输出.客户端.控制台输出("收到服务端返回消息, {}", JSONObject.toJSONString(登录响应数据包));
