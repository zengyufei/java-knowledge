...
public class F2_消费者 {
    ...
    public static void 监听队列接收mq消息() {
        ...
        try {
            ...
            信道.basicConsume(F0_常量.队列名称, autoAck,
                    (consumerTag, message) -> {
                        System.out.println(F2_消费者.class.getSimpleName() + " =" + 信道.hashCode());
                        final String val = new String(message.getBody(), StandardCharsets.UTF_8);
                        System.out.println(F2_消费者.class.getSimpleName() + "接收到消息: " + val);
                        try {
                         ...
                         */
                        System.out.println(F2_消费者.class.getSimpleName() + "消费完毕: " + val);
                        信道.basicAck(message.getEnvelope().getDeliveryTag(), false);
                    },
                    consumerTag -> System.out.println(F2_消费者.class.getSimpleName() + "消息消费被中断,取消消费消息"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void 关闭() {
        ...
        if (信道 != null && 信道.isOpen()) {
            try {
                System.out.println(F2_消费者.class.getSimpleName() + " 关闭=" + 信道.hashCode());
                信道.close();
